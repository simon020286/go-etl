...
func init() {
	pipeline.RegisterTriggerType("webhook", func(name string, config map[string]any) (core.Step, error) {
		...
		// Register webhook endpoint on the main APIServer router
		registry := web.GetWebhookRegistry()
		router := registry.GetRouter()
		if router != nil {
			router.HandleFunc("/webhook/"+path, func(w http.ResponseWriter, r *http.Request) {
			...
			slog.Info("Webhook endpoint registered", slog.String("path", "/webhook/"+path), slog.String("method", strings.ToUpper(method)))
		} else {
			slog.Warn("APIServer router not available, webhook endpoint not registered", "path", path)
		}
